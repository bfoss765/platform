...
impl Drive {
    ...
    pub(super) fn add_indices_for_top_index_level_for_contract_operations_v1(
        ...
        for (name, sub_level) in index_level.sub_levels() {
            ...
            // property keeps its single key. Null timestamps are never bucketed
            // (they keep their single null entry).
            let index_keys: Vec<DriveKeyInfo> = match sub_level.time_range() {
                Some(transform) if !document_top_field.is_empty() => match &document_top_field {
                    // Estimated-cost path (size only): the real timestamp isn't
                    // available, so assume the worst case of `overlap_factor`
                    // overlapping buckets.
